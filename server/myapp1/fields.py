...
class CascadeOneToOneField(OneToOneField):
    def __init__(self, to, *args, **kwargs):
        kwargs.setdefault("null", True)
        kwargs.setdefault("blank", True)
        kwargs.setdefault("on_delete", models.CASCADE)
...
class MonthYearField(models.DateField):
    """
    A DateField that only expects and shows Month/Year.
    """

    def __init__(self, *args, **kwargs):
        kwargs.setdefault("null", True)
        kwargs.setdefault("blank", True)
        kwargs.setdefault("help_text", "Format: YYYY-MM")
        kwargs.setdefault("default", default_month_start)
        super().__init__(*args, **kwargs)
        ...
        if isinstance(value, str):
            ...
            for fmt in ["%Y-%m"]:
                try:
                    ...
                    continue
            raise ValidationError("Invalid format. Use YYYY-MM!!!")
        return super().to_python(value)
